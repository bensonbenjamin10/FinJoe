...
export async function fetchSystemContext(): Promise<string> {
  if (cachedContext && Date.now() - cachedAt < CONTEXT_CACHE_TTL_MS) return cachedContext;
...
export function clearSystemContextCache() {
  cachedContext = null;
  cachedAt = 0;
}
...
export async function fetchSystemData(): Promise<{
  campuses: CampusInfo[];
